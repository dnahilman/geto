...
bun run dev:server          # API on :3000 (or PORT)
...
- geto is published on the host at **`http://localhost:${PORT}`** (the container
  always listens on 3000 internally; `PORT` only sets the host mapping). If host
  3000 is taken, set `PORT=3100` in `.env` and open `http://localhost:3100`.
...
All optional — `docker-compose.yml` sets sensible defaults you can override via a
...
| `PORT` | `3100` | Host port geto is published on (container always listens on 3000) |
